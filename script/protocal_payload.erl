...
%% 解析float
encode_double(Integer) when is_integer(Integer) ->
  Float = Integer*1.0,
  <<Float:64/float>>;
encode_double(Float) when is_float(Float) ->
  <<Float:64/float>>.
decode_double(<<Float:64/float, Data/binary>>)  ->
  {Float, Data}.
