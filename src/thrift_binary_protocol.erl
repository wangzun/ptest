...
read(This0, string) ->
  ...
  case erlang:get(fromclient) of
    true ->
      read_string(This1, Sz);
    _ ->
      read_data(This1, Sz)
  end.

read_string(This, 0) -> {This, {ok, <<>>}};
read_string(This = #binary_protocol{transport = Trans}, Len) when is_integer(Len) andalso Len > 0 ->
  {NewTransport, {ok, Result}} = thrift_transport:read(Trans, Len),
  case unicode:characters_to_list(Result, utf8) of
    {error, _, _} ->
    ...
    _List ->
      %%string_check(List),
      {This#binary_protocol{transport = NewTransport}, {ok, Result}}
  end.
